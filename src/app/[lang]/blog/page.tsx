...
const articleMeta = [
  { id: 1, date: '14 Jun 2024', readTime: '8 min', img: 'https://images.unsplash.com/photo-1579684453423-f84349ef60b0?w=800&q=80&fit=crop', featured: true },
  { id: 2, date: '8 Jun 2024', readTime: '6 min', img: 'https://images.unsplash.com/photo-1555664424-778a1e5e1b48?w=800&q=80&fit=crop', featured: false },
  { id: 3, date: '1 Jun 2024', readTime: '7 min', img: 'https://images.unsplash.com/photo-1576086213369-97a306d36557?w=800&q=80&fit=crop', featured: false },
  { id: 4, date: '24 May 2024', readTime: '10 min', img: 'https://images.unsplash.com/photo-1528360983277-13d401cdc186?w=800&q=80&fit=crop', featured: false },
  { id: 5, date: '17 May 2024', readTime: '9 min', img: 'https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=800&q=80&fit=crop', featured: false },
];
...
export default async function BlogPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const featured = { ...articleMeta[0], ...t.articles[0] };
  const rest = articleMeta.slice(1).map((m, i) => ({ ...m, ...t.articles[i + 1] }));
          ...
          <div style={{ padding: '4rem', background: 'var(--surface-container-low)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <p style={{ fontSize: '0.75rem', color: 'var(--outline)' }}>
                {featured.date} · {featured.readTime} {t.reading_suffix}
              </p>
              <Link href={`/${lang}/blog/${featured.id}`} style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 700, color: 'var(--primary)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          {rest.map((article) => (
            <article key={article.id} style={{ borderBottom: '1px solid var(--outline-variant)', paddingBottom: '3rem' }}>
              <div style={{ aspectRatio: '16/9', overflow: 'hidden', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem' }}>
                <img src={article.img} alt={article.title} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform var(--transition-slow)' }} className="blog-img" />
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <p style={{ fontSize: '0.75rem', color: 'var(--outline)' }}>{article.date} · {article.readTime}</p>
                <Link href={`/${lang}/blog/${article.id}`} style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--primary)', fontFamily: 'var(--font-body)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
